require('ansicolor').nice;                  // console font color management
const { Command } = require('commander');   // for parsing CLA arguments
const ccxt = require('ccxt');
const asTable = require('as-table');        // for tabular dispaly of financial data
const log = require('ololog').configure({ locate: false, time: { yes: true, format: 'utc' }});  // log of trading activity
const program = new Command();

// exclude symbols that may be temporarily suspended from trading
// assumes a manually maintained list /config/symbol_excluded
...
const compareAll = (oB, ids, symbol, exchanges) => {
    ...
    if (imax != null && jmax != null) {
      ...
      // this is where the min profit threshold is set 0.6 = 6%
...
program
  ...
  .action(async (args) => {
    ...
    for (let id of ids) {
      ...
      // save it in array under its id for future use
